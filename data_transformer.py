...
from utils.proj_adaptive_softmax import ProjectedAdaptiveLogSoftmax, Projection
...
class PositionwiseFF(nn.Module):
    def __init__(self, d_model, d_inner, dropout, pre_lnorm=False):
        ...
        self.CoreNet = nn.Sequential(nn.Linear(d_model, d_inner), nn.ReLU(inplace=True), nn.Dropout(dropout),
...
class RelMultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, tgt_len=None, ext_len=None, mem_len=None,
                 pre_lnorm=False, moe=False, moe_num_expert=64, moe_top_k=2):
...
class RelPartialLearnableMultiHeadAttn(RelMultiHeadAttn):
    ...
    def forward(self, w, r, r_w_bias, r_r_bias, attn_mask=None, mems=None):
        ...
        if attn_mask is not None and attn_mask.any().item():
            if attn_mask.dim() == 2:
                attn_score = attn_score.float().masked_fill(attn_mask[None, :, :, None].bool(), -float('inf')).type_as(
                    attn_score)
            elif attn_mask.dim() == 3:
                attn_score = attn_score.float().masked_fill(attn_mask[:, :, :, None].bool(), -float('inf')).type_as(
                    attn_score)
...
class DecoderLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, **kwargs):
        ...
        if kwargs.get('moe') is False:
            ...
            self.pos_ff = CustomizedMoEPositionwiseFF(d_model, d_inner, dropout, pre_lnorm=kwargs.get('pre_lnorm'),
    ...
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner, dropout, dropatt, tie_weight=True,
        ...
        if attn_type == 0:  # the default attention
            for i in range(n_layer):
                self.layers.append(
                    RelPartialLearnableDecoderLayer(n_head, d_model, d_head, d_inner, dropout, tgt_len=tgt_len,
                                                    ext_len=ext_len, mem_len=mem_len, dropatt=dropatt,
                                                    pre_lnorm=pre_lnorm, moe=moe,
                                                    moe_num_expert=moe_num_expert, moe_top_k=moe_top_k))
        elif attn_type == 1:  # learnable embeddings
            for i in range(n_layer):
                self.layers.append(RelLearnableDecoderLayer(n_head, d_model, d_head, d_inner, dropout, tgt_len=tgt_len,
                                                            ext_len=ext_len, mem_len=mem_len, dropatt=dropatt,
                                                            pre_lnorm=pre_lnorm, moe=moe,
                                                            moe_num_expert=moe_num_expert, moe_top_k=moe_top_k))
            ...
            for i in range(n_layer):
                self.layers.append(
                    DecoderLayer(n_head, d_model, d_head, d_inner, dropout, dropatt=dropatt, pre_lnorm=pre_lnorm,
    ...
    def _forward(self, dec_inp, mems=None):
        ...
        if self.attn_type == 0:  # default
            ...
            for i, layer in enumerate(self.layers):
                ...
                core_out = layer(core_out, pos_emb, self.r_w_bias, self.r_r_bias, dec_attn_mask=dec_attn_mask,
                                 mems=mems_i)
                hids.append(core_out)
        elif self.attn_type == 1:  # learnable
            core_out = self.drop(word_emb)
...
if __name__ == '__main__':
    ...
    for div_val in [1, 2]:
        for d_embed in [200, 100]:
            model = MemTransformerLM(args.n_token, args.n_layer, args.n_head, args.d_model, args.d_head, args.d_inner,
                                     args.dropout, dropatt=args.dropout, tie_weight=True, d_embed=d_embed,
                                     div_val=div_val, tie_projs=tie_projs, pre_lnorm=True, tgt_len=tgt_len,
                                     ext_len=ext_len, mem_len=mem_len, cutoffs=cutoffs, attn_type=0).to(device)
